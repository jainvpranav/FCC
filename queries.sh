...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON teams.team_id = games.opponent_id OR teams.team_id = games.winner_id WHERE year=2014 ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT(name) FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id WHERE winner_goals > opponent_goals ORDER BY name")"
